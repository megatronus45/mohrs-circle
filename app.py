...
with cols[0]:
    ...
    t_xy = st.number_input("Enter τₓᵧ:", 
                           min_value=-1e15,
                           max_value=1e15,
                           value = 0.0,
                           step=2.0,
...
with cols[2]:
    ...
    if st.session_state.show_circle:
        ...
        circle_x, circle_y, sx_prime, sy_prime, t_prime, theta_slider = slider(r, s_avg, theta_slider, s_x, s_y, t_xy)
        fig = o_plot(r, s_avg, s_max, s_min, s_x, s_y, t_xy, circle_x, circle_y, sx_prime, sy_prime, t_prime)
...
with cols[1]:
    ...
    st.text_input("Stress (σₓ') at angle θ",
                    value = f"{sx_prime:.2f}",     
                    help = "Stress in x when the principal angle is changed",
                    disabled = True)
    
    st.text_input("Stress (σᵧ') at angle θ",
                    value = f"{sy_prime:.2f}",     
